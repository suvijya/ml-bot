...
def speak(message, language="en"):
    """Function to output speech in the specified language (default is English)."""
    try:
        # Choose the language code based on the detected language
        tts = gTTS(text=message, lang=language, slow=False)
        
        # Save the audio and play it
        tts.save("response.mp3")
        os.system("start response.mp3")  # On Windows
        # Use 'afplay response.mp3' for macOS or 'mpg321 response.mp3' for Linux
    except Exception as e:
        print(f"Error in TTS: {e}")
        print("Speaking:", message)  # Fallback to console output
...
vosk_model_en_path = "vosk-model-en-in-0.5"  # English model path
vosk_model_hi_path = "vosk-model-small-hi-0.22"  # Hindi model path

# Load the models
if not os.path.exists(vosk_model_en_path) or not os.path.exists(vosk_model_hi_path):
    raise Exception("Vosk models not found. Download and place them in the specified paths.")

vosk_model_en = Model(vosk_model_en_path)
vosk_model_hi = Model(vosk_model_hi_path)

def detect_language():
    """Detect if the response is in English or Hindi based on keywords."""
    p = pyaudio.PyAudio()
    stream = p.open(rate=16000, channels=1, format=pyaudio.paInt16, input=True, frames_per_buffer=8192)
    
    # Use a general model or loop through both models until keywords are detected
    recognizer_en = KaldiRecognizer(vosk_model_en, 16000)
    recognizer_hi = KaldiRecognizer(vosk_model_hi, 16000)
    
    speak("Please say 'yes' or 'no' in English, or 'हाँ' or 'नहीं' in Hindi.")
    ...
    while True:
        ...
        if recognizer_en.AcceptWaveform(data):
            ...
            answer_en = result_en.get("text", "")
            if "yes" in answer_en or "no" in answer_en:
                detected_language = "en"
                break

        if recognizer_hi.AcceptWaveform(data):
            result_hi = json.loads(recognizer_hi.Result())
            answer_hi = result_hi.get("text", "")
            if "हाँ" in answer_hi or "नहीं" in answer_hi:
...
def vosk_yes_no_confirmation():
    """Use Vosk for yes or no confirmation in detected language."""
    language = detect_language()  # Detect language first
    
    # Set up the PyAudio stream again after language detection
    p = pyaudio.PyAudio()
    stream = p.open(rate=16000, channels=1, format=pyaudio.paInt16, input=True, frames_per_buffer=8192)
    ...
    if language == "en":
        ...
        recognizer = KaldiRecognizer(vosk_model_hi, 16000)
        speak("कृपया हां या ना कहें।")
    else:
    ...
    while True:
        ...
        if recognizer.AcceptWaveform(data):
            ...
            print(f"Detected response: {answer}")
            
            if language == "en" and ("yes" in answer or "no" in answer):
                response = "yes" if "yes" in answer else "no"
            elif language == "hi" and ("हाँ" in answer or "नहीं" in answer):
                response = "हाँ" if "हाँ" in answer else "नहीं"
            else:
                response = None
            ...
            if response:
                ...
                stream.close()
                p.terminate()
                return response == ("yes" if language == "en" else "हाँ")

            speak("Please respond with yes or no." if language == "en" else "कृपया हां या ना कहें।")
